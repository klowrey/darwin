...
void phasespace_comm() {

	/*
	printf("\tCalibration Vector: \n");
	...
	*/
	...
	double marker_holder[MARKER_COUNT*3];
	...
	int count=0;
	mjMSG msg;
	while( mjw_getEvent(ps_procID, &msg, 0)==0 )
	{
		...
		if(n > 0) {
			count++;
			// temp buffer for maker data before
			// calibrated data is put into ps_pose
			for(int i = 0; i < MARKER_COUNT; i++) {
				if(markers[i].cond > PHASESPACE_CONF) {
					marker_holder[3*i+0]=markers[i].x/1000.0;
					marker_holder[3*i+1]=markers[i].y/1000.0;
					marker_holder[3*i+2]=markers[i].z/1000.0;
				}
				else {
					marker_holder[3*i+0]=sqrt(-1.0);
					marker_holder[3*i+1]=sqrt(-1.0);
					marker_holder[3*i+2]=sqrt(-1.0);
				}
			}
			for(int i = 0; i < MARKER_COUNT; i++) {
				tt_rotVecMat(ps_pose+3*i, marker_holder+3*i, TRT);
				ps_pose[3*i+0] = TRc[0]+ps_pose[3*i+0];
				ps_pose[3*i+1] = TRc[1]+ps_pose[3*i+1];
				ps_pose[3*i+2] = TRc[2]+ps_pose[3*i+2];
			}

			if (count > 2000) { // about every 4 seconds?
				for(int i = 0; i < MARKER_COUNT; i++) {
					if(markers[i].cond > PHASESPACE_CONF) {
						//printf("(%d) %f %f %f\n", i, markers[i].x, markers[i].y, markers[i].z);
						printf("(%d) %f %f %f\n", i, ps_pose[3*i+0], ps_pose[3*i+1], ps_pose[3*i+2]);
					}
					else {
						printf("(%d) nan  nan  nan\n", i);
					}
				}
				printf("\n");
				count = 0;
